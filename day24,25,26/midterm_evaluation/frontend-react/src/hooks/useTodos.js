...
export const useTodos = (filter = "all") => {
  ...
  const filteredTodos = todoStats.filteredTodos(result.data, filter);
  // const sortedTodos = todoStats.sortTodos(filteredTodos);
  const stats = todoStats.calculateStats(result.data);
...
export const useToggleTodo = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ todoId, isCompleted }) =>
      todoAPI.toggleTodo(todoId, isCompleted),
