...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    // 첫 번째 제네릭 타입은 관련 테이블, 두 번째 제네릭 타입은 관련 테이블 PK 의 타입
    ...
    // - JpaRepository 인터페이스에 내장된 메서드만으로는 해결이 안되는 경우, (raw query 를 작성해야 함)
    // - 단, Jpa는 테이블 명이 아니라 객체 위주로 돌아가니 객체(Entity) 이르을 사용해야 함
    // - nativeQuery 옵션
    ...
    // 참고. CASE WHEN (조건식) THEN (결과1) ELSE (결과2) END (SQL문) => CASE WHEN 문
